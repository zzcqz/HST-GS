...
PerGaussianRenderCUDA(
	...
	const __half* __restrict__ sampled_T, const __half* __restrict__ sampled_ar,
	const float* __restrict__ bg_color,
	const float2* __restrict__ points_xy_image,
	const float4* __restrict__ conic_opacity,
	const float* __restrict__ colors,
	...
	if (global_bucket_idx >= (uint32_t)B) return;
	...
	if (bucket_idx_in_tile * 32 >= (int)max_contrib[tile_id]) return;
	...
	for (int i = 0; i < BLOCK_SIZE + 31; ++i) {
		if (i % 32 == 0) {
			for (int ch = 0; ch < C; ++ch) {
				...
				Shared_sampled_ar[ch * 32 + block.thread_rank()] = 0.0f;
				if (i + (int)block.thread_rank() < BLOCK_SIZE) {
					Shared_sampled_ar[ch * 32 + block.thread_rank()] = __half2float(sampled_ar[shift]);
				}
			}
			const uint32_t local_id = i + block.thread_rank();
			const uint2 pix = {pix_min.x + local_id % BLOCK_X, pix_min.y + local_id / BLOCK_X};
			for (int ch = 0; ch < C; ++ch) {
				Shared_pixels[ch * 32 + block.thread_rank()] = 0.0f;
			}
			if (local_id < BLOCK_SIZE && pix.x < W && pix.y < H) {
		...
		if (valid_splat && valid_pixel && my_warp.thread_rank() == 0 && idx < BLOCK_SIZE) {
			T = __half2float(sampled_T[global_bucket_idx * BLOCK_SIZE + idx]);
			int ii = i % 32;
			for (int ch = 0; ch < C; ++ch) ar[ch] = -Shared_pixels[ch * 32 + ii] + Shared_sampled_ar[ch * 32 + ii];
			T_final = final_Ts[pix_id];
			last_contributor = n_contrib[pix_id];
			for (int ch = 0; ch < C; ++ch) dL_dpixel[ch] = dL_dpixels[ch * H * W + pix_id];
		}
		...
		if (valid_splat && valid_pixel && 0 <= idx && idx < BLOCK_SIZE) {
			...
			reg_mean_x += tmp_x;
			reg_mean_y += tmp_y;
			reg_mean_abs_x += fabs(tmp_x);
			reg_mean_abs_y += fabs(tmp_y);
	...
	if (valid_splat) {
		...
		const unsigned int group_mask = __match_any_sync(active_mask, gaussian_idx);
		...
		if (group_size == 1) {
			if ((int)lane == leader) {
				...
				for (int ch = 0; ch < C; ++ch) {
					atomicAdd(&dL_dcolors[gaussian_idx * C + ch], reg_colors[ch]);
				}
			}
		} else {
			...
			for (int ch = 0; ch < C; ++ch) {
				...
				if ((int)lane == leader) {
					atomicAdd(&dL_dcolors[gaussian_idx * C + ch], color_sum);
				}
			}
		}
	}
...
void BACKWARD::render(
	...
	const __half* sampled_T, const __half* sampled_ar,
...
{
	if (B <= 0) {
		// Native 3DGS tile-based backward (HTS off): no FastGS bucket snapshots.
