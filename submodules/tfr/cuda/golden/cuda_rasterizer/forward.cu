...
// Min of q(u,v)=a*u^2+2*b*u*v+c*v^2 over rectangle [u0,u1]x[v0,v1].
	...
	bool antialiasing)
{
	...
	// If base opacity is below forward alpha threshold, this Gaussian can never contribute.
	...
	// radii is set only after we confirm tiles_touched > 0 (see below).
	depths[idx] = p_view.z;
	...
	// Tight tile box from the alpha cutoff ellipse: alpha = opacity * exp(-0.5 * q) >= 1/255.
	...
	// Hierarchical pruning: reject whole 2x2 super-tiles if their rectangle cannot reach alpha threshold.
	...
	for (int sty = st_min_y; sty < st_max_y; ++sty) {
		for (int stx = st_min_x; stx < st_max_x; ++stx) {
			...
			const float u0 = rx0 - point_image.x;
			const float u1 = rx1 - point_image.x;
			const float v0 = ry0 - point_image.y;
			const float v1 = ry1 - point_image.y;
			const float min_q = quadMinRect(conic.x, conic.y, conic.z, u0, u1, v0, v1);
			if (min_q > q)
				continue;

			touched += (uint32_t)((sub_tile_max_x - sub_tile_min_x) * (sub_tile_max_y - sub_tile_min_y));
		}
	}
